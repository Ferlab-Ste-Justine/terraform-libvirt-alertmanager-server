...
This module provision prometheus' alertmanager instances on kvm.
...
- **data_volume_id**: Id for an optional separate disk volume to attach to the vm on alertmanager's data path
- **libvirt_network**: Parameters to connect to libvirt networks. Note that while the server will bind on all interfaces for web and peer traffic, only the first interface will be advertised to cluster peers, including macvtap interfaces. Each entry has the following keys:
...
- **macvtap_interfaces**: List of macvtap interfaces to connect the vm to if you opt for macvtap interfaces. Note that while the server will bind on all interfaces for web and peer traffic, only the first interface will be advertised to cluster peers, including libvirt networks. Each entry in the list is a map with the following keys:
